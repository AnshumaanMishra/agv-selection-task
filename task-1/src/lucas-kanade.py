...
# yKernel = xKernel.T  # Transpose for Y-direction
    ...
    for i, ele in enumerate(corners):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                tempmat2 = prev_frame[x + j - 2: x + j + 3, y + i - 2 : y + i + 3]
                print("Tempmat = ", tempmat1)
                print("Tempmat2 = ", tempmat2)
                print("Sobel = ", np.int64(cv2.Sobel(tempmat1, cv2.CV_64F, 1, 0, ksize=3, scale=1)))
                print("Sobel = ", np.int64(cv2.Sobel(tempmat1, cv2.CV_64F, 0, 1, ksize=3, scale=1)))
                Ix[i][j] = sum(sum(tempmat1 * xKernel))
                Iy[i][j] = sum(sum(tempmat1 * yKernel))
                It[i][j] = mat2[i][j] - mat[i][j]
        ...
        # xMean, xStd = np.min(Ix), np.max(Ix)
        # yMean, yStd = np.min(Iy), np.max(Iy)
        # tMean, tStd = np.min(It), np.max(It)
        # Ix = -1 + 2 * np.int64(min_max_normalization(Ix, xMean, xStd))
        # Iy = -1 + 2 * np.int64(min_max_normalization(Iy, yMean, yStd))
        # It = -1 + 2 * np.int64(min_max_normalization(It, tMean, tStd))
        # print(f"Ixa : \n{Ixa}\n, Iya: \n{Iya}\n")
